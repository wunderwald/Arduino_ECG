...
def trial(
    ...
    if modality == "Intero":
        ...
        while True:
            ...
            # get the ecg signal and peaks from the last 5 seconds, resampled to 1000hz
            signal, peaks, millis = parameters['ecgThread'].getSignalAndPeaks(length_s=5, samplingRate_hz=1000)
            ...
            if np.isnan(bpm).any() or (bpm is None) or (bpm.size == 0):
                message = visual.TextStim(
                    parameters["win"],
                    height=parameters["textSize"],
                    text="Not enough peaks for BPM calculation. Check ECG.",
                    color="red",
                )
                message.draw()
                parameters["win"].flip()
