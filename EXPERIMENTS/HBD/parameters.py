...
from ecg import EcgMonitorThread
import serial

ARD_PORT = '/dev/cu.usbmodem14201'
BAUD = 115200
FULLSCREEN = False
...
def getParameters(
    ...
    # set up artduino
    parameters['ard'] = serial.Serial(port=ARD_PORT, baudrate=BAUD)

    # set up ecg monitor thread
    parameters['ecgThread'] = EcgMonitorThread(ard=parameters['ard'])
    parameters['ecgThread'].start()
    ...
    if language == "english":
        parameters["texts"] = english(
            device=device, exteroception=exteroception
        )
